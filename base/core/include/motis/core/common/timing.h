...
#define MOTIS_START_TIMING(_X) \
  auto _X##_start = std::chrono::high_resolution_clock::now()
#define MOTIS_STOP_TIMING(_X) auto _X##_stop = std::chrono::high_resolution_clock::now()
...
#define MOTIS_GET_TIMING_MS(_X)                           \
  (std::chrono::duration_cast<std::chrono::milliseconds>( \
       std::chrono::high_resolution_clock::now() - _X##_start)     \
       .count())
